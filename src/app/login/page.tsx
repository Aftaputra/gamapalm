...
import {
  ...
  uploadFile,
} from "@/lib/api"; // UBAH ke "@/lib/api" kalau memang API kamu di sana
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            Selamat datang, <strong>{user.name}</strong> dari <strong>{user.company}</strong>.
          </p>
        </div>
        <a
          href="/ndvi-upload"
    ...
    async function loadData() {
      try {
        ...
        if (!foundUser.company) {
          setError("Profil perusahaan Anda belum lengkap (nama perusahaan kosong).");
          setLoading(false);
          return;
        }
    ...
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-slate-100">
        <div className="bg-white p-8 rounded-xl shadow-lg text-center max-w-md">
          ...
          <button
            onClick={() => router.push("/login/perusahaan")}
                  ...
                  projectCriteria.map((criterion) => (
                    <div key={criterion.id}>
                      ...
                      {criterion.status === "Revisi Diperlukan" &&
                        criterion.auditorNotes && <AuditorFeedbackCard notes={criterion.auditorNotes} />}
